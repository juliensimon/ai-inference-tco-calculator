...
- **DeepSeek peak/off-peak pricing took effect August 2026.** Peak hours are 01:00–04:00 and 06:00–10:00 UTC at 2x the off-peak rate. The library stores the **off-peak** (17h/day) cache-miss rate, with peak rates in each model's `notes`.
...
- **A promo needs both a hard end date and a published list price to be stored at list.** Gemini 3.7/3.6 Flash qualify ("$0.75 through December 31, 2026. $1.50 starting January 1, 2027") so the library stores $1.50/$7.50. GPT-5.6 Sol does not: its page shows only $4/$20 and says the promo runs "at least through November 21, 2026" — a soft floor with no list price to fall back to — so the **effective** $4/$20 is stored.
